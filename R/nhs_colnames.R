...
#' @param grep filter colnames by grep() function
#' @param exact filter colnames by exact match
#' @param ex_grep exclude by grep() function
#' @param ex_exact exclude by exact match
#' @param combine combine colnames together
...
#'
#' nhs_colnames(files = demo,view = T,
...
#'
#' }
nhs_colnames <- function(files,order=FALSE,
                         grep=NULL,exact=NULL,
                         ex_grep=NULL,ex_exact=NULL,
                         combine=NULL,view=FALSE) UseMethod('nhs_colnames')
#' @rdname nhs_colnames
#' @method nhs_colnames character
#' @export
nhs_colnames.character <- function(files,order=FALSE,
                            grep=NULL,exact=NULL,
                            ex_grep=NULL,ex_exact=NULL,
    ...
    if (!is.null(ex_grep)){
        ex_grep <- paste0(ex_grep,collapse = '|')
...
#' @export
nhs_colnames.list <- function(files,order=FALSE,
                                   grep=NULL,exact=NULL,
                                   ex_grep=NULL,ex_exact=NULL,
                                   combine=NULL,view=FALSE){
